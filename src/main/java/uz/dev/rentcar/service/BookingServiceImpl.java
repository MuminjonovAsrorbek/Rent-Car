...
public class BookingServiceImpl implements BookingService {
    ...
    public BookingDTO createBooking(BookingCreateDTO dto, User currentUser) {
        ...
        log.info("Booking created successfully for user: {}, car: {}, pickup: {}, return: {}", currentUser.getId(), car.getId(), dto.getPickupDate(), dto.getReturnDate());
        Booking save = bookingRepository.save(booking);
        notificationService.createNotification(currentUser, "Your booking has been created successfully.", NotificationTypeEnum.INFO, save);
    ...
    public BookingDTO cancelBooking(Long id, User currentUser) {
        ...
        notificationService.updateBookingStatus(currentUser, "Your booking has been cancelled successfully.", NotificationTypeEnum.WARNING, id, BookingStatusEnum.CANCELLED);
        return bookingMapper.toDTO(save);
    }
    ...
    public BookingDTO confirmBooking(Long id) {
        ...
        notificationService.updateBookingStatus(booking.getUser(), "Your booking has been confirmed successfully.", NotificationTypeEnum.INFO, id, BookingStatusEnum.CONFIRMED);
        return bookingMapper.toDTO(save);
    }
    ...
    public BookingDTO completeBooking(Long id) {
        ...
        notificationService.updateBookingStatus(booking.getUser(), "Your booking has been completed successfully.", NotificationTypeEnum.INFO, id, BookingStatusEnum.COMPLETED);
        return bookingMapper.toDTO(save);
    }

    @Override
    @Transactional
    public BookingDTO extendBooking(Long id, BookingExtendDTO dto, User currentUser) {
        Booking booking = bookingRepository.getByIdOrThrow(id);
        if (isAdmin(currentUser) && !Objects.equals(booking.getUser().getId(), currentUser.getId())) {
            throw new SecurityException("You do not have permission to extend this booking.");
        }
        if (booking.getStatus() != BookingStatusEnum.CONFIRMED) {
            throw new InvalidRequestException("Only CONFIRMED bookings can be extended.", HttpStatus.BAD_REQUEST);
        }
        LocalDateTime oldReturnDate = booking.getReturnDate();
        LocalDateTime newReturnDate = dto.getNewReturnDate();
        if (newReturnDate == null || !newReturnDate.isAfter(oldReturnDate)) {
            throw new InvalidRequestException("New return date must be after the current return date.", HttpStatus.BAD_REQUEST);
        }
        List<BookingStatusEnum> activeStatuses = List.of(BookingStatusEnum.PENDING, BookingStatusEnum.CONFIRMED);
        if (bookingRepository.existsOverlappingBookingForExtension(id, booking.getCar().getId(), oldReturnDate, newReturnDate, activeStatuses)) {
            throw new CarNotAvailableException("This car is not available for the selected extension period.", HttpStatus.CONFLICT);
        }
        long additionalHours = ChronoUnit.HOURS.between(oldReturnDate, newReturnDate);
        long pricePerHour = booking.getCar().getPricePerDay() / 24;
        long additionalPrice = pricePerHour * additionalHours;
        booking.setReturnDate(newReturnDate);
        booking.setTotalPrice(booking.getTotalPrice() + additionalPrice);
        Booking updatedBooking = bookingRepository.save(booking);
        log.info("Booking ID {} has been extended to {}. New total price: {}", id, newReturnDate, updatedBooking.getTotalPrice());
        notificationService.createNotification(updatedBooking.getUser(), "Your booking has been successfully extended. New return date is: " + newReturnDate.toString(), NotificationTypeEnum.INFO, updatedBooking);
